...
public class OrderInfoController extends BaseController {
	...
	@InitBinder("productName")
	public void initBinderproductName(WebDataBinder binder) {
		binder.setFieldDefaultPrefix("productName.");
	}
	...
	public void add(@Validated OrderInfo orderInfo, BindingResult br,
		...
		orderInfo.setPrintFile(this.handleFileUpload(request, "printFileFile"));
		orderInfo.setHandleFile(this.handleFileUpload(request, "handleFileFile"));
	...
	public void userAdd(OrderInfo orderInfo, BindingResult br,
		...
		int productId = orderInfo.getProductName().getProductId();
		float price = productService.getProduct(productId).getPrice();
		orderInfo.setTotalMoney(price * orderInfo.getOrderNumber());
		
		orderInfo.setPrintFile(this.handleFileUpload(request, "printFileFile"));
		orderInfo.setHandleFile(this.handleFileUpload(request, "handleFileFile"));
		
		if(orderInfo.getPrintFile().equals("")) {
			message = "请上传需要打印的文件！";
			writeJsonResponse(response, success, message);
			return ;
		}
	...
	public void list(@ModelAttribute("productName") Product productName,String telephone,@ModelAttribute("orderStateObj") OrderState orderStateObj,@ModelAttribute("userObj") UserInfo userObj,String orderTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (telephone == null) telephone = "";
		if (orderTime == null) orderTime = "";
		if(rows != 0)orderInfoService.setRows(rows);
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(productName, telephone, orderStateObj, userObj, orderTime, page);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(productName, telephone, orderStateObj, userObj, orderTime);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("productName") Product productName,String telephone,@ModelAttribute("orderStateObj") OrderState orderStateObj,@ModelAttribute("userObj") UserInfo userObj,String orderTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (telephone == null) telephone = "";
		if (orderTime == null) orderTime = "";
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(productName, telephone, orderStateObj, userObj, orderTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(productName, telephone, orderStateObj, userObj, orderTime);
	    /*获取到总的页码数目*/
	    int totalPage = orderInfoService.getTotalPage();
	...
	public String userFrontlist(@ModelAttribute("productName") Product productName,String telephone,@ModelAttribute("orderStateObj") OrderState orderStateObj,@ModelAttribute("userObj") UserInfo userObj,String orderTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (telephone == null) telephone = "";
		if (orderTime == null) orderTime = "";
	...
	public void update(@Validated OrderInfo orderInfo, BindingResult br,
		...
		String printFileFileName = this.handleFileUpload(request, "printFileFile");
		if(!printFileFileName.equals(""))orderInfo.setPrintFile(printFileFileName);
		String handleFileFileName = this.handleFileUpload(request, "handleFileFile");
		if(!handleFileFileName.equals(""))orderInfo.setHandleFile(handleFileFileName);
		try {
	...
	public void OutToExcel(@ModelAttribute("productName") Product productName,String telephone,@ModelAttribute("orderStateObj") OrderState orderStateObj,@ModelAttribute("userObj") UserInfo userObj,String orderTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(telephone == null) telephone = "";
        if(orderTime == null) orderTime = "";
        List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(productName,telephone,orderStateObj,userObj,orderTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "OrderInfo信息记录"; 
        String[] headers = { "订单id","订单商品","订单数量","订单总金额","联系电话","订单状态","下单用户","下单时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<orderInfoList.size();i++) {
        	OrderInfo orderInfo = orderInfoList.get(i); 
        	dataset.add(new String[]{orderInfo.getOrderId() + "",orderInfo.getProductName().getProductName(),orderInfo.getOrderNumber() + "",orderInfo.getTotalMoney() + "",orderInfo.getTelephone(),orderInfo.getOrderStateObj().getStateName(),orderInfo.getUserObj().getName(),orderInfo.getOrderTime()});
        }
        /*
